...
# Lazy imports to avoid OpenCV errors during Streamlit health checks
if "STREAMLIT_SERVER_HEALTH_CHECK" not in os.environ:
    import cv2
...
class VideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        global cv2, shield
        # Lazy import if not already loaded
        if cv2 is None or shield is None:
            import cv2
            from magic_shield import MagicShield
            shield = MagicShield()
